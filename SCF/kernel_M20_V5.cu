...
const bool IsInitP_cross = true;
const int Init_P_cross_it = 49;
#define threads 512
...
public:
    ...
    //Array* rho3_t;
    Array* temp;
    Array* omega;//not equal to Net.omega
    Array* rho;
    //Array* Pi;
    Double** rho2_p_gpu;
    Double Z0, Zb;
...
public:
    ...
    void Cal_Zm(int m);
    void z_cross_multi(int& ni, Double& p1, Double& p2, CrossPoint* c, int i2, Array* _t1, Array* _t2, int j,Double temp_multi);
...
void SCF_system::Net_init() {
    ...
    for (int j = 0; j < Net.chain_num; j++) {
        ...
        if (IsDouble) {
            ...
            if (IsInitP_cross) {
                sprintf(filename, "p_init_chain%d_cid%d_it%d.dat", j,ct->c1->id,Init_P_cross_it);
                ifstream ifs00(filename, std::ios::binary | std::ios::in);
                ifs00.read((char*)cpu_temp->p, sizeof(Double) * Volume);
                ifs00.close();
                ct->c1->h->aCopy(*cpu_temp);
                ct->c1->h->aLn(-1.0);
                sprintf(filename, "p_init_chain%d_cid%d_it%d.dat", j, ct->c2->id, Init_P_cross_it);
                ifstream ifs1(filename, std::ios::binary | std::ios::in);
                ifs1.read((char*)cpu_temp->p, sizeof(Double) * Volume);
                ifs1.close();
                ct->c2->h->aCopy(*cpu_temp);
...
void SCF_system::Ab_Init() {
    if (IsDouble) {
        ifstream ifs5("omega_Ab_init_D.dat", std::ios::binary | std::ios::in);
...
void SCF_system::Init() {
    ...
    int shape[] = {Ab.Num, Net.chain_num};
    cpu_temp2 = new Array(2, shape, false);
...
void SCF_system::z_cross_multi(int& ni, Double& p1, Double& p2, CrossPoint* c, int i2, Array* _t1, Array* _t2, int j,Double temp_multi)
{
    Chain* ch;
...
void SCF_system::Run_an_iter(bool output) {
    ...
    for (int j = 0; j < Net.chain_num; j++) {
        ...
        dG_cnf2->p[j] = -log(Net.chain[j].Z2ij) + log(Net.chain[j].Qij) + log(Ab.Zb)+dG_cnf_ex;
    ...
    Array chain_state2(1, 192 * 3 * (NANO_M + 1), false);
    ...
    for (int m = 0; m < bond_max_num + 1; m++) {
        ...
        cpu_temp2->aCopy(*dGij);
        SaveData(cpu_temp2->p, "dGij.dat", Net.chain_num * Ab.Num);
        Cal_link_p(dGij, bij, bij_temp, bi, Net.chain_num, Ab.Num, bij_iter);
        cpu_temp2->aCopy(*bij);
        sprintf(string_temp, "bij_%d.dat", m);
        SaveData(cpu_temp2->p, string_temp, Net.chain_num * Ab.Num);
        cpu_temp2->aCopy(*bi);
        ...
        for (int j = 0; j < Net.chain_num; j++) {
            ...
            chain_state2.p[m * (Net.chain_num) * 3 + j * 3 + 0] = Net.chain[j].p_i_m[0]->p[m];
            chain_state2.p[m * (Net.chain_num) * 3 + j * 3 + 1] = Net.chain[j].p_i_m[1]->p[m];
            chain_state2.p[m * (Net.chain_num) * 3 + j * 3 + 2] = Net.chain[j].p_i_m[2]->p[m];
        }

    }
    Save_var("p_i_m.dat", chain_state2);
    Double ln_H0;
    ln_H0 = ln_Hm->p[0];
    for (int m = 0; m < bond_max_num + 1; m++) {
        if (ln_H0 < ln_Hm->p[m]) {
    ...
    Ab.n1 = t;
    Ab.n0 = Ab.Num - t;
    Array chain_state(1, 192 * 3 + 4, false);

    for (int j = 0; j < Net.chain_num; j++) {
        Ab.n1 -= Net.chain[j].Pi->p[2];
        Ab.n0 -= Net.chain[j].Pi->p[1];
        chain_state.p[j * 3 + 0] = Net.chain[j].Pi->p[0];
        chain_state.p[j * 3 + 1] = Net.chain[j].Pi->p[1];
        chain_state.p[j * 3 + 2] = Net.chain[j].Pi->p[2];
    }
    cout << "Ab:" << Ab.n0 << ";" << Ab.n1 << endl;
    Net.rho->SetValue(0.0);
    chain_state.p[192 * 3] = Ab.n0;
    chain_state.p[192 * 3 + 1] = Ab.n1;
    Save_var("Ab0.dat", *Ab.rho0);
    Save_var("Ab1.dat", *Ab.rho1);

    Ab.rho->aWAdd(*Ab.rho0, *Ab.rho1, Ab.n0, Ab.n1);
    chain_state.p[192 * 3 + 2] = 0;
    chain_state.p[192 * 3 + 3] = 0;
    t1->SetValue(0.0, -1, 0);
    t2->SetValue(0.0, -1, 0);
    for (int j = 0; j < Net.chain_num; j++) {
        //if (j == 0) {
        ...
        for (int k = 0; k < 3; k++) {
            ...
            Net.rho->aAdd(*Net.chain[j].rho[k]);
        }
        chain_state.p[192 * 3 + 2] += Net.chain[j].Pi->p[1];
        chain_state.p[192 * 3 + 3] += Net.chain[j].Pi->p[2];
    ...
    Save_var("Net_state.dat", chain_state);
    Save_var("Ab2.dat", *t1);
    Save_var("Ab3.dat", *t2);
    ...
    for (int j = 0; j < Net.chain_num; j++) {
        for (int ic = 0; ic < 2; ic++) {
            ...
            c->p_n->SetValue(0.0);
            Double temp_multi;
            ch = Net.cross_to_chain[c->id][0];
            if (ch->c1->id != c->id) {
                temp_multi = ch->Zi012[2]->aSum(*temp);
            }
            else {
                temp_multi = ch->Zj012[2]->aSum(*temp);
            }
            temp_multi = 1.0 / temp_multi;
            n1 = 0;
            for (int i1 = 0; i1 < 3; i1++) {
                ...
                for (int i2 = 0; i2 < 3; i2++) {
                    ...
                    z_cross_multi(n2, p1, p2, c, i2, t1, t2, j,temp_multi);
                    n3 = n2 + 1;
                    for (int i3 = 0; i3 < 3; i3++) {
                        if (Net.cross_to_chain[c->id][n3]->id == Net.chain[j].id) {
                            n3++;
                        }
                        z_cross_multi(n3, p2, p3, c, i3, t2, t3, j, temp_multi);
                        n4 = n3 + 1;
                        for (int i4 = 0; i4 < 3; i4++) {
                            if (Net.cross_to_chain[c->id][n4]->id == Net.chain[j].id) {
                                n4++;
                            }
                            z_cross_multi(n4, p3, p4, c, i4, t3, t4, j, temp_multi);
                            n5 = n4 + 1;
                            for (int i5 = 0; i5 < 3; i5++) {
                                if (Net.cross_to_chain[c->id][n5]->id == Net.chain[j].id) {
                                    n5++;
                                }
                                z_cross_multi(n5, p4, p5, c, i5, t4, t5, j, temp_multi);
                                t5->aMulti(p5);
                                c->p_n->aAdd(*t5);
    ...
    Net.omega_n->aWAdd(*Net.rho, *Ab.rho, v0, v01);
    Net.omega_n->aWAdd(*NP.WCA, 1.0, 1.0);
    Net.omega->aWAdd(*Net.omega_n, 1.0 - step, step);
    Net.omega_n->aWAdd(*Net.rho, *Ab.rho, v01, v1);
    Net.omega_n->aWAdd(*NP.WCA, 1.0, 1.0);
    Ab.omega->aWAdd(*Net.omega_n, 1.0 - step, step);
    Ab.Cal_Z(NP);
    out_var[it * 2] = F;
    out_var[it * 2 + 1] = P_unbind;
    it++;
}
void CrossPoint::h2p() {
    p->aExp(*h, -1.0);
...
int main(int argc, char* argv[])
{
    ...
    Double dG_cnf_ex = strtod(argv[8], NULL);
    Double volume_ratio = strtod(argv[9], NULL);
    if (set_bond0 < -0.1) {
        IsUnbind = false;
    }

    Double V1 = V0 / volume_ratio / volume_ratio;
    Double V01 = 1.0 / volume_ratio;
    SCF_system scf;
    scf.dG_NP = dG_NP;
    scf.dG_Net = dG_Net;
