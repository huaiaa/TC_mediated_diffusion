...
optParser.add_option("-P","--Particle", action="store", type=int, dest="Particle",
...
date='0416'
if Particle_num>1:
    sim_name=date+dying+'g{0}p{1}Ab{2}m55S{3}{4}N{5}__'.format(graft, phi, antibody_phi, size,netB,Particle_num)
else:
    sim_name=date+dying+'g{0}p{1}Ab{2}m55S{3}{4}__'.format(graft, phi, antibody_phi, size,netB)
if not os.path.exists('simulation/{0}'.format(sim_name)):
    ...
    P_ord_num = np.array(
        np.loadtxt('particles/{0}{1}m55_direaction/npnumber.txt'.format(dying, graft)).astype('int').tolist())
    ...
    init_particle_pos = np.array(
        [[0.0, 0.0, 0.0], [2 * mesh_len, 2 * mesh_len, 2 * mesh_len], [2 * mesh_len, 2 * mesh_len, -2 * mesh_len],
         [2 * mesh_len, -2 * mesh_len, 2 * mesh_len], [2 * mesh_len, -2 * mesh_len, -2 * mesh_len]]).reshape(-1, 3)
    init_particle_pos = np.array([[0.0, 0.0, 0.0]])
    particle_min_dis = []
    ord_num = []
    particle_pos = []
    i = 0
